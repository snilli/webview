...
const ProductListItem = forwardRef<HTMLDivElement, ProductListItemProps>(
	(
		...
		return (
			...
			>
				{isNew && isPopular ? (
					<span className="absolute right-7 top-7 z-20 text-tiny font-semibold text-default-400">NEW</span>
				) : null}
				<Button
					isIconOnly
				...
				<div className="flex flex-col gap-3 px-1">
					...
					{description && !isPopular ? <p className="text-small text-default-500">{description}</p> : null}
					{rating !== undefined ? (
						<RatingRadioGroup
							hideStarsText
							isReadOnly
